...
function toHtml5QrcodeCameraScanConfig(config: Html5QrcodeScannerConfig): Html5QrcodeCameraScanConfig {
  return {
    fps: config.fps,
    qrbox: config.qrbox,
...
function toHtml5QrcodeFullConfig(config: Html5QrcodeConfigs, verbose: boolean | undefined): Html5QrcodeFullConfig {
  return {
...
export class Html5QrcodeScanner {
   ...
   */
  public constructor(elementId: string, config: Html5QrcodeScannerConfig | undefined, verbose: boolean | undefined) {
   ...
   */
  public render(qrCodeSuccessCallback: QrcodeSuccessCallback, qrCodeErrorCallback?: QrcodeErrorCallback) {
  ...
  private createConfig(config: Html5QrcodeScannerConfig | undefined): Html5QrcodeScannerConfig {
    if (config) {
      if (!config.fps) {
  ...
  private renderCameraSelection(cameras: Array<CameraDevice>) {
    ...
    cameraSelectionContainer.innerText = `Select Camera (${cameras.length}) &nbsp;`;
    ...
    cameraActionStartButton.addEventListener('click', (_) => {
      ...
      $this
        .html5Qrcode!.start(cameraId, toHtml5QrcodeCameraScanConfig($this.config), $this.qrCodeSuccessCallback!, $this.qrCodeErrorCallback!)
        .then((_) => {
  ...
  private createSectionSwap() {
    ...
    swithToFileBasedLink.innerText =
      this.currentScanType === Html5QrcodeScanType.SCAN_TYPE_CAMERA ? TEXT_IF_CAMERA_SCAN_SELECTED : TEXT_IF_FILE_SCAN_SELECTED;
  ...
  private setStatus(statusText: string, scannerStatus?: Html5QrcodeScannerStatus) {
    if (!scannerStatus) {
  ...
  private setHeaderMessage(messageText: string, scannerStatus?: Html5QrcodeScannerStatus) {
    if (!scannerStatus) {
      scannerStatus = Html5QrcodeScannerStatus.STATUS_DEFAULT;
    }
  ...
  private getFileScanInput(): HTMLInputElement {
    return <HTMLInputElement>document.getElementById(this.getFileScanInputId())!;
  }
...
class InternalHtml5QrcodeConfig implements InternalHtml5QrcodeConfig {
  ...
  private constructor(config: Html5QrcodeCameraScanConfig | undefined, logger: Logger) {
    this.logger = logger;
   ...
   */
  static create(config: Html5QrcodeCameraScanConfig | undefined, logger: Logger): InternalHtml5QrcodeConfig {
...
export class Html5Qrcode {
   ...
   */
  public constructor(elementId: string, configOrVerbosityFlag: boolean | Html5QrcodeFullConfig | undefined) {
    if (!document.getElementById(elementId)) {
      throw `HTML Element with id=${elementId} not found`;
    }
    ...
    this.logger = new BaseLoggger(this.verbose);
    this.qrcode = new Html5QrcodeShim(this.getSupportedFormats(configOrVerbosityFlag), this.verbose, this.logger);
  ...
  public start(
    ...
    const width = element.clientWidth ? element.clientWidth : Constants.DEFAULT_WIDTH;
    ...
    if (isShadedBoxEnabled) {
      ...
      if (qrboxSize < Constants.MIN_QR_BOX_SIZE) {
        throw "minimum size of 'config.qrbox' is" + ` ${Constants.MIN_QR_BOX_SIZE}px.`;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        navigator.mediaDevices
          ...
          .then((stream) => {
            $this
              .onMediaStreamReceived(stream, internalConfig, areVideoConstraintsEnabled, width, qrCodeSuccessCallback, qrCodeErrorCallback!)
              .then((_) => {
   ...
   *      load errors.
   */
  public scanFile(imageFile: File, /* default=true */ showImage?: boolean): Promise<string> {
   ...
   */
  public scanFileV2(imageFile: File, /* default=true */ showImage?: boolean): Promise<Html5QrcodeResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      inputImage.onload = () => {
        ...
        const element = document.getElementById(this.elementId)!;
        const containerWidth = element.clientWidth ? element.clientWidth : Constants.DEFAULT_WIDTH;
   ...
   * @throws error if the scanning is not in running state.
   */
  public applyVideoConstraints(videoConstaints: MediaTrackConstraints): Promise<any> {
    if (!videoConstaints) {
   ...
   * TODO(mebjas): Deprecate the verbosity boolean flag completely.
   */
  private getSupportedFormats(configOrVerbosityFlag: boolean | Html5QrcodeFullConfig | undefined): Array<Html5QrcodeSupportedFormats> {
   ...
   */
  private setupUi(width: number, height: number, internalConfig: InternalHtml5QrcodeConfig): void {
    const qrboxSize = internalConfig.qrbox!;
    if (qrboxSize > height) {
    ...
    const qrRegion = shouldShadingBeApplied ? this.getShadedRegionBounds(width, height, qrboxSize) : defaultQrRegion;
   ...
   */
  private scanContext(qrCodeSuccessCallback: QrcodeSuccessCallback, qrCodeErrorCallback: QrcodeErrorCallback): boolean {
    try {
  ...
  private createVideoConstraints(cameraIdOrConfig: Html5QrcodeIdentifier): MediaTrackConstraints | undefined {
    if (typeof cameraIdOrConfig == 'string') {
  ...
  private createCanvasElement(width: number, height: number, customId?: string): HTMLCanvasElement {
  ...
  private getShadedRegionBounds(width: number, height: number, qrboxSize: number): QrcodeRegionBounds {
  ...
  private possiblyInsertShadingElement(element: HTMLElement, width: number, height: number, qrboxSize: number) {
  ...
  private insertShaderBorders(shaderElem: HTMLDivElement, width: number, height: number, top: number, side: number, isLeft: boolean) {
...
declare global {
  interface Window {
    ...
    Html5Qrcode: any;
    makeHtml5QrcodeScanner: (elementId: string, config?: Html5QrcodeScannerConfig, verbose?: boolean) => Html5QrcodeScanner;
    makeHtml5QrCode: (elementId: string, configOrVerbosityFlag?: boolean | Html5QrcodeFullConfig | undefined) => Html5Qrcode;
  }
}
